...
window.addEventListener('scroll', () => {
  ...
  // Throttle adicional para reducir carga
...
function typeWrite() {
  ...
  // Efecto de color dinámico RGB HD
  twEl.style.color = colors[pi];
  twEl.style.textShadow = `0 0 15px ${colors[pi]}60, 0 0 30px ${colors[pi]}40, 0 0 45px ${colors[pi]}20`;
  twEl.style.fontWeight = '700';
  
  deleting ? ci-- : ci++;
  if (!deleting && ci === cur.length) { deleting = true; setTimeout(typeWrite, 2000); return; }
  if (deleting && ci === 0) { deleting = false; pi = (pi + 1) % phrases.length; }
  setTimeout(typeWrite, deleting ? 80 : 100);
}
if (twEl) {
  twEl.style.transition = 'color 0.2s ease, text-shadow 0.2s ease, font-weight 0.2s ease';
  twEl.style.fontFamily = 'var(--font-head)';
  typeWrite();
}
...
// Fallback inmediato: si el navegador móvil no dispara IntersectionObserver
// a tiempo, los datos nunca deben quedarse en 0.
function ensureStatsVisible() {
  Object.entries(statTargets).forEach(([id, value]) => {
    const el = document.getElementById(id);
    if (el && (!el.textContent || el.textContent.trim() === '0')) el.textContent = value;
  });
}
setTimeout(ensureStatsVisible, 1200);
...
// ── SCROLL REVEAL MÁXIMO OPTIMIZADO ────────────────────────────────────
...
// Las reseñas deben quedar visibles también en móviles/táctiles.
// Si una animación fue deshabilitada o el observer llegó tarde, no las ocultamos.
setTimeout(() => {
  document.querySelectorAll('#reviewsGrid .review-card, #reviewsGrid .review-card.reveal').forEach(card => {
    card.classList.add('visible');
  });
}, 900);

// ── 3D TILT — desactivado para mejor rendimiento en CPU débil ────
// El efecto tilt 3D consume mucho CPU en cada mousemove. Se mantiene el hover normal del CSS.
// document.querySelectorAll('[data-tilt]') → sin tilt

// ── MOUSE SHINE — desactivado para rendimiento en CPU débil ─────
// El radial-gradient dinámico en cada mousemove era costoso en Intel Celeron.
...
// Reseñas enviadas desde este navegador se conservan para que no desaparezcan
// al recargar la página. Las reseñas base del HTML siempre permanecen visibles.
const REVIEWS_STORAGE_KEY = 'fonsec_reviews_v2';
...
function renderSavedReviews() {
  ...
  try {
    ...
    saved.reverse().forEach(r => {
      ...
      card.innerHTML = `<div class="review-stars">${stars}</div><p class="review-text">"${escapeReviewText(r.comment)}"</p><div class="review-author"><div class="review-avatar">⭐</div><div><span class="review-name">Cliente Fonsec</span><span class="review-service">Calificación: ${Math.max(1, Math.min(5, Number(r.rating) || 5))}/5</span></div></div>`;
...
if (submitRatingBtn) {
  submitRatingBtn.addEventListener('click', () => {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem(REVIEWS_STORAGE_KEY) || '[]');
      saved.push({ rating: selectedRating, comment, createdAt: Date.now() });
      localStorage.setItem(REVIEWS_STORAGE_KEY, JSON.stringify(saved.slice(-30)));
    } catch (_) {}
    const avatars = ['👨','👩','🧑','👦','👧','🧔','👩‍💻','👨‍💼'];
...
document.querySelectorAll('a[href^="#"]').forEach(a => {
  a.addEventListener('click', e => {
    ...
    // Smooth scroll básico sin efectos adicionales
    window.scrollTo({
      top: targetPosition,
      behavior: 'smooth'
    });
...
// ── ACTIVE NAV ON SCROLL OPTIMIZADO MÁXIMO ─────────────────────
const allSections = document.querySelectorAll('section[id]');
let lastScrollYNav = window.pageYOffset;
let tickingNav = false;
let navScrollTimeout;

// Optimización extrema para navegación activa
window.addEventListener('scroll', () => {
  lastScrollYNav = window.pageYOffset;
  if (!tickingNav) {
    window.requestAnimationFrame(() => {
      const sy = lastScrollYNav;
  ...
  // Throttle extremo para reducir carga
...
const SERVICE_DATA = {
  ...
  'pc-diagnostico': {
    ...
    sub: 'Precio fijo $20.000 — Para todos los equipos sin excepción.',
    items: [
  ...
  'pc-discos': {
    icon: '💽', title: 'Revisión de Disco SSD / HDD',
    sub: 'Comprobamos la salud, velocidad y estado real de tu unidad de almacenamiento.',
    items: [
      '🩺 Lectura de salud y vida útil del SSD o HDD',
      '⚡ Prueba de velocidad y rendimiento',
      '🔍 Detección de sectores dañados y errores',
      '💾 Revisión de espacio, particiones y sistema',
      '📁 Orientación para respaldo y recuperación de datos',
      '🚀 Recomendación clara: reparar, optimizar o reemplazar',
    ]
  },
  'pc-windows': {
  ...
  'pc-bateria': {
    icon: '🔋', title: 'Cambio de Batería de PC',
    sub: 'Batería nueva = vida extendida para tu computadora.',
    items: [
      '🔍 Diagnóstico del estado actual de la batería CMOS',
      '🔋 Batería de calidad garantizada para laptops y PC',
      '🔧 Reemplazo profesional sin dañar otros componentes',
      '⚡ Calibración de la nueva batería en BIOS',
      '📊 Prueba de duración y autonomía después del cambio',
      '✅ Garantía en la batería instalada',
    ]
  },
  'pc-fuente': {
    icon: '⚡', title: 'Arreglo de Fuentes ATX',
    sub: 'Fuentes de poder reparadas = energía estable y segura.',
    items: [
      '🔍 Diagnóstico completo de la fuente de alimentación',
      '🔧 Reemplazo de capacitores electrolíticos dañados',
      '⚡ Reparación de reguladores de voltaje (PWM)',
      '🌡️ Limpieza interna profunda de polvo y residuos',
      '💨 Cambio de pasta térmica en disipadores',
      '📊 Prueba de voltajes de rieles (+12V, +5V, +3.3V)',
      '🔌 Revisión y limpieza de conectores SATA, PCIe, ATX',
      '⚙️ Ajuste de ventiladores para mejor flujo de aire',
      '📋 Informe detallado del estado de la fuente',
  ...
  'con-diagnostico': {
    ...
    sub: 'Diagnóstico completo por solo $20.000.',
    items: [
...
(function init4KUpgrade() {
  ...
  /* ── 5. EFECTOS DE HOVER LIGEROS ─────────────────────
     Se elimina el tilt 3D y los destellos automáticos:
     ambos ejecutaban trabajo en cada movimiento del mouse. */
  ...
  /* ── 7. SCROLL PROGRESS BAR — REMOVIDO ───────────────
     Quitado por petición: la barrita de arriba molestaba  */


  /* ── 8. SECTION HEADERS — GLOW AURA ON ENTER ──────── */
  ...
  /* ── 9. WORK CARDS — STAGGER ENTRANCE ─────────────── */
  document.querySelectorAll('.trabajo-card').forEach((card, i) => {
    card.style.transitionDelay = (i * 0.08) + 's';
  });


  /* ── 10. NAV LINKS — NEON RIPPLE ON HOVER ───────────── */
...
(function perfPatch() {
  ...
  if (isTouch) {
    // IMPORTANTE: no clonar/reemplazar tarjetas en móvil.
    // Hacer cloneNode aquí desconectaba los IntersectionObserver y dejaba
    // los contadores en 0 y las reseñas con opacity:0.
    // En táctil simplemente no registramos efectos de hover pesados.
  }
  ...
  if (fps === 'low') {
    ...
    if (pBg) {
      const ps = Array.from(pBg.querySelectorAll('.particle'));
       ps.forEach((p, i) => { if (i >= 5) p.remove(); });
    }
  ...
  /* ── 11. VIDEOS — cargar solo al acercarse y pausar fuera ── */
  ...
  document.querySelectorAll('.trabajo-video').forEach(video => {
